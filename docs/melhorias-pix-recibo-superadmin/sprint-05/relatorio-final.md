...
1. **SEGURANÇA (prioridade alta):** a senha do banco de produção (`***REDACTED***`) foi exposta na mensagem do commit público `74c2a05`. Fazer **Reset database password** no painel do Supabase (Settings → Database → Reset database password) e atualizar a env `PGPASSWORD` no Render + redeploy.
2. **Rotina de suporte:** a senha da conta de suporte vive em `SUPPORT_ADMIN_PASSWORD` no Render; para trocar: atualizar a env e reiniciar (o seed recria/atualiza no startup). O usuário pode excluir a conta na tela de usuários — ela será recriada no próximo reinício (comportamento por design).
