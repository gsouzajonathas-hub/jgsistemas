...
D-15 | **PLANO ABANDONADO — pivô para "opção 1"**: NÃO reescrever o backend como Edge Functions (Deno). Manter FastAPI no Render + Supabase Postgres + disco persistente/`/data` + email Resend; frontend na Vercel; Supabase vira apenas Postgres (sem Auth/RLS/Edge Functions). Commit 8da47cb "Prepara deploy (opcao 1)" remove supabase-sync e código Supabase do frontend. Este sprintx (D-01..D-14, 7 sprints/52 tasks de Edge Functions) fica descontinuado; a entrega passou a ser via DEPLOY.md (runbook) + hardening no código FastAPI | seguir o plano Edge Functions | usuário escolheu "opçaõ 1" no commit 8da47cb após F5 (deploy direto sem reescrita Deno)
```
